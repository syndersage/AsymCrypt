...
public class ElGamalSignature {
  ...
  public static byte[] sign(byte[] digest, ElGamalKeys keys) {
    ...
    } while (randomValue.compareTo(intModulus.subtract(BigInteger.ONE)) < 0
        | randomValue.gcd(intModulus.subtract(BigInteger.ONE)).compareTo(BigInteger.ONE) != 0);
    ...
    BigInteger encryptedMessage = inverseRandom.multiply(
        intDigest.subtract(intPrivate.multiply(encryptedGenerator))
            .mod(intModulus.subtract(BigInteger.ONE))).mod(intModulus.subtract(BigInteger.ONE));

    return Numbers.concatenateArrays(Numbers.i2osp(encryptedGenerator, keys.modulus.length),
        Numbers.i2osp(encryptedMessage, keys.modulus.length));
  }
  ...
  public static byte[] verify(ElGamalKeys keys, byte[] signature) {
    ...
    BigInteger encryptedMessage = new BigInteger(
        Arrays.copyOfRange(signature, keys.modulus.length, signature.length));

    BigInteger result = intPublic.modPow(encryptedGenerator, intModulus)
        .multiply(encryptedGenerator.modPow(encryptedMessage, intModulus)).mod(intModulus);
