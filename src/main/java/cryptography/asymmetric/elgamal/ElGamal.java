...
public class ElGamal {
  ...
  public static byte[] encrypt(byte[] data, ElGamalKeys keys, OAEP paddingParams) {
      ...
      case "PKCS#1-OAEP" -> {
        ...
        if (chunkSize <= 0) {
          throw new IllegalArgumentException(
              " Key size for PKCS#1-OAEP must be at least: " + (bytesForPadding * 8 + 1) + " bits");
        }
      }
      case "None" -> chunkSize = keys.byteKeyLength - 1;
      default ->
          throw new IllegalArgumentException(" Invalid padding name: " + paddingParams.padding);
    }
    ...
    for (int i = 0; i < splittedData.length; i++) {
      ...
      encryptedMessageChunks[i] = Numbers.i2osp(
          Numbers.os2ip(messageChunk).multiply(tempPublic.modPow(randomValue, tempModulus))
              .mod(tempModulus),
          keys.byteKeyLength);
    }
  ...
  public static byte[] decrypt(byte[] data, ElGamalKeys keys, OAEP paddingParams) {
    ...
    byte[][] splittedData = Numbers.splitArray(
        Arrays.copyOfRange(data, keys.byteKeyLength, data.length), keys.byteKeyLength);
    ...
    for (int i = 0; i < decryptedData.length; i++) {
      ...
      decryptedData[i] = Numbers.i2osp(paddedChunk.multiply(
              encryptedGenerator.modPow(tempModulus.subtract(BigInteger.ONE).subtract(tempPrivate),
                  tempModulus)).mod(tempModulus),
          keys.byteKeyLength);
      switch (paddingParams.padding) {
        case "PKCS#1-OAEP" -> decryptedData[i] = OAEP.unwrap(decryptedData[i], paddingParams);
        case "None" ->
            decryptedData[i] = Arrays.copyOfRange(decryptedData[i], 1, keys.byteKeyLength);
      }
    }
