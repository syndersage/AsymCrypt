...
public class ElGamalKeys {
  ...
  public ElGamalKeys(int keyLength) throws IllegalArgumentException {
    if (keyLength < MIN_KEY_LENGTH | keyLength > MAX_KEY_LENGTH) {
      throw new IllegalArgumentException("Key length (in bits) have to in range from " + MIN_KEY_LENGTH + " to " + MAX_KEY_LENGTH);
    }
  ...
  public ElGamalKeys(BigInteger base, BigInteger modulus) throws IllegalArgumentException {
    if (modulus.bitLength() < MIN_KEY_LENGTH | modulus.bitLength() > MAX_KEY_LENGTH + Numbers.randomValueSize + 1) {
      throw new IllegalArgumentException(" Elgamal key length min and max length respectively: " + MIN_KEY_LENGTH + ", " + MAX_KEY_LENGTH);
    }
  ...
  public void setPersonalKeys(BigInteger publicKey, BigInteger privateKey) throws IllegalArgumentException {
    if (publicKey.compareTo(BigInteger.ONE) <= 0 | publicKey.compareTo(new BigInteger(modulus)) >= 0
      | privateKey.compareTo(BigInteger.ONE) <= 0 | privateKey.compareTo(new BigInteger(modulus)) >= 0) {
  ...
  public void setPersonalKeys(BigInteger personalKey, boolean isPublicKey) throws IllegalArgumentException {
    if (personalKey.compareTo(BigInteger.ONE) <= 0 | personalKey.compareTo(new BigInteger(modulus)) >= 0) {
      throw new IllegalArgumentException((isPublicKey ? " Public" : "Private") + " key must be greater than 1 and less than modulus");
    }
    if (isPublicKey) {
