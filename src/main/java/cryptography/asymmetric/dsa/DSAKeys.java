...
public class DSAKeys {
  ...
  public DSAKeys(int modulusLength, int order) throws IllegalArgumentException {
    ...
    if (contain != 1) {
      throw new IllegalArgumentException(
          " Modulus size and group order must one of the following pairs:" + Arrays.deepToString(
              ABLE_PAIRS));
    }
  ...
  public DSAKeys(BigInteger base, BigInteger modulus, BigInteger order)
  ...
  public void setPersonalKeys() {
    ...
    personalPublicKey = new BigInteger(1, base).modPow(tempPrivate, new BigInteger(1, modulus))
        .toByteArray();
  ...
  public void setPersonalKeys(BigInteger publicKey, BigInteger privateKey)
    ...
    if (publicKey.compareTo(BigInteger.ONE) <= 0
        | publicKey.compareTo(new BigInteger(1, modulus)) >= 0
        | privateKey.compareTo(BigInteger.ONE) <= 0
        | privateKey.compareTo(new BigInteger(1, groupOrder)) >= 0) {
  ...
  public void setPublicKey(BigInteger publicKey) throws IllegalArgumentException {
    if (publicKey.compareTo(new BigInteger(1, modulus)) >= 0
        | publicKey.compareTo(BigInteger.ONE) <= 0) {
  ...
  public void setPrivateKey(BigInteger privateKey) throws IllegalArgumentException {
    if (privateKey.compareTo(new BigInteger(1, groupOrder)) >= 0
        | privateKey.compareTo(BigInteger.ONE) <= 0) {
      throw new IllegalArgumentException("Incorrect public key size");
    } else {
      personalPrivateKey = privateKey.toByteArray();
