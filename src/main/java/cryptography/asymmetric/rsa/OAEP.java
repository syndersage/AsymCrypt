...
public class OAEP implements Padding {
  ...
  /**
   * Паддинд по стандарту PKCS#1 v2.2 шифруемого алгоритмом RSA сообщения. <p>В RFC8017 требуется
   * seed фиксированного размера (длины хэш алгоритма) - для этого берется хэш от seed произвольного
   * размера при помощи того же алгоритма хэширования</p>
   *
   * @param message сообщение, которое планируется шифровать
   * @param params  параметры паддинга: seed - добавление недетерминированности, bytesKeyLength -
   *                длина ключа (модуля) в RSA, label - дополнительный текст для проверки
   *                целостности
   ...
   * @param params        паддинга: label - дополнительный текст для проверки целостности,
   *                      bytesKeyLength - длина ключа (модуля) в RSA (знание seed при удалении
   *                      паддинга не нужно)
  ...
  public static byte[] unwrap(byte[] paddedMessage, OAEP params)
    ...
    if (paddedMessage[0] != 0) {
      throw new IllegalArgumentException("Padding is invalid",
          new IllegalArgumentException("0x00 byte at beginning of padded message not found"));
    }
    byte[] maskedSeed = Arrays.copyOfRange(paddedMessage, 1, 1 + bytesDigestLength);
    byte[] maskedDataBlock = Arrays.copyOfRange(paddedMessage, 1 + bytesDigestLength,
        ...
        if (dataBlock[i] != 0) {
          if (dataBlock[i++] != 1) {
            throw new IllegalArgumentException(
                "Padding string and message does not separated with 0x01");
          } else {
            break;
          }
        }
